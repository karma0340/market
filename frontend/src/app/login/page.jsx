...
import { Package, Mail, Lock, ShieldCheck, ArrowRight, Eye, EyeOff, Github } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="pointer-events-none fixed inset-0">
        ...
        <div className="absolute bottom-0 right-0 w-[400px] h-[300px] bg-[#0070FF] opacity-[0.05] blur-[100px] rounded-full" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        ...
        <div className="bg-[#0A0F1E] border border-[#1E293B] rounded-2xl p-6 sm:p-8 shadow-[0_24px_80px_rgba(0,0,0,0.6)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Field label="Email Address" icon={Mail} type="email" value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} placeholder="name@company.com" />
            <Field label="Password"      icon={Lock} type="password" value={formData.password} onChange={e => setFormData(p => ({ ...p, password: e.target.value }))} placeholder="Enter your password" />
          ...
          <div className="flex flex-col gap-3 justify-center w-full overflow-hidden">
            <GoogleLogin onSuccess={handleGoogle} onError={() => toast.error('Google login failed')}
              theme="filled_black" size="large" width="370" text="continue_with" shape="rectangular" />
            
            <button type="button" onClick={() => {
              window.location.href = `https://github.com/login/oauth/authorize?client_id=${process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID}&redirect_uri=${window.location.origin}/login&scope=user:email`;
          ...
          <p className="mt-6 text-center text-sm text-[#52525B]">
            New here?{' '}
            <Link href="/register" className="font-bold text-[#00D2FF] hover:text-[#38BDF8] transition-colors">Create an account</Link>
          </p>
        </div>
